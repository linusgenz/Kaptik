...
Item {
    ...
    // Current APM value indicator (dot) - nur bei Hover sichtbar
    Rectangle {
        id: currentDot
        visible: false  // Wird nur bei Hover angezeigt
        width: 8
        height: 8
        radius: width / 2
        color: graphColor
        ...
        x: (currentPosition / duration) * parent.width - width/2
        y: parent.height - ((root.currentApm / maxApm) * parent.height) - height/2
    }
